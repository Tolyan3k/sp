...
int SDS()                                         /*подпр.обр.пс.опер.DS    */
{
  ...
  {
    /* то: занулим RX.OP_RX.B2D2 */
    RX.OP_RX.B2D2 = 0;
    STXT (4, 0);                                       /*формирование TXT-карты  */
  }
  else if (TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[0]=='B' ||
           TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[0]=='C' ||
           TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[0]=='0') {
    STXT(0, 2);
  }
  else                                            /*иначе                   */
...

  {
                                          ...
                                          /* сообщение об ошибке    */
    {
      ...
      for (I1 = 0; I1 <= DL_ASSTEXT; I1++) {
        int fread_size = fread(ASSTEXT[I1], ASSCARD_SIZE, 1, fp);
        if (fread_size) {
          fgetc(fp);
          continue;
        }
...
main1:
  ...
  */
  ...
  {                                   /*                        */
    ...
    memcpy(TEK_ISX_KARTA.BUFCARD, ASSTEXT[I1], ASSCARD_SIZE);
    /*переход при отсутствии метки на CONT1,*/
    if (TEK_ISX_KARTA.STRUCT_BUFCARD.METKA[0] == ' ') goto CONT1;
    /*иначе:                  */
